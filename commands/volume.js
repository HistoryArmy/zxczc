...
module.exports.run = async (client, message, args) => {
  ...
  if (!channel)
    return message.reply(
      "Sorry but you have to be on an audio channel to play music",
      message.channel
    );
  ...
  if (isNaN(args[0]))
    return message.channel
      .send(":notes: You just need to specify the number.")
      .catch(err => console.log(err));
  if (parseInt(args[0]) > 150 || args[0] < 0)
    return message
      .reply(
        "You cannot adjust the volume above 150 or below 0",
        message.channel
      )
      .catch(err => console.log(err));
